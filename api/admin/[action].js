...
async function handleSendShift(req, res, supabase) {
  ...
  const emailHtml = `<h2>Your Shift Details — ${event.title}</h2><p><strong>Date:</strong> ${formatDate(event.event_date)}</p><p><strong>Time:</strong> ${formatTime(event.start_time)} – ${formatTime(event.end_time)}</p><p><strong>Location:</strong> ${event.location}, ${event.city}</p>${event.meeting_point ? `<p><strong>Meeting Point:</strong> ${event.meeting_point}</p>` : ''}${event.supervisor_name ? `<p><strong>Supervisor:</strong> ${event.supervisor_name}${event.supervisor_phone ? ` · ${event.supervisor_phone}` : ''}</p>` : ''}${event.pay_rate ? `<p><strong>Pay Rate:</strong> ${event.pay_rate}</p>` : ''}${event.dress_code ? `<p><strong>Dress Code:</strong> ${event.dress_code}</p>` : ''}<hr/><p style="color:#888;font-size:12px">Questions? <a href="mailto:crew@joinvanda.co">crew@joinvanda.co</a></p>`
...
async function handleSendSurvey(req, res, supabase) {
  ...
  const smsBody = `Hi ${worker.first_name}! Thanks for working ${eventTitle}. Please share your feedback: ${surveyUrl}`
  const emailHtml = `<h2>How was your shift? — ${eventTitle}</h2><p>Hi ${worker.first_name},</p><p>Thank you for working with Vanda! Please take a moment to share your experience.</p><p><a href="${surveyUrl}" style="background:#c8ff00;color:#000;padding:10px 20px;border-radius:20px;text-decoration:none;font-weight:bold;">Complete Your Survey →</a></p>`

  await Promise.allSettled([
    worker.phone ? sendSms(worker.phone, smsBody) : Promise.resolve(),
    worker.email ? sendEmail({ to: worker.email, subject: `How was your shift? — ${eventTitle}`, html: emailHtml }) : Promise.resolve(),
  ])
...
async function handleNotifyWorkers(req, res, supabase) {
  ...
  const formatShortDate = (d) => d ? new Date(d + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) : ''

  let sent = 0
  for (const worker of workers) {
    if (!worker.phone) continue
    try {
      await sendSms(worker.phone, `Hey ${worker.first_name}! A new shift is available in ${event.city}: ${event.title} on ${formatShortDate(event.event_date)}. Claim it at ${siteUrl}/shifts`)
      sent++
    } catch (e) {
      console.error(`SMS failed for worker ${worker.id}:`, e.message)
    }
  }
