  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
      ...
      <div className="px-6 pb-16 max-w-5xl mx-auto w-full">
        <div className="text-[#ffffff] font-semibold text-xs tracking-widest uppercase mb-6">Two Service Tiers</div>
        <div className="grid md:grid-cols-2 gap-6">
          {SERVICE_TIERS.map(tier => (
            <div key={tier.title} className={`border rounded-2xl p-6 ${tier.title === 'Managed Labor' ? 'border-[#ffffff]/30 bg-[#ffffff]/[0.02]' : 'border-[#1e1e1e]'}`}>
              ...
              <ul className="space-y-2">
                {tier.features.map((f, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm">
                    <span className="text-[#ffffff] mt-1 text-xs">+</span>
      ...
      <div className="px-6 pb-20 max-w-5xl mx-auto w-full grid md:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            {ORGANIZER_STEPS.map(({ step, title, body }) => (
              <div key={step} className="flex gap-4">
                <div className="text-[#ffffff] font-extrabold text-sm w-8 shrink-0 pt-1">{step}</div>
                <div>
        ...
        {/* For Workers */}
        <div>
          <div className="text-[#ffffff] font-semibold text-xs tracking-widest uppercase mb-6">For Workers</div>
          <div className="space-y-8">
      ...
      <div className="px-6 py-16 border-t border-[#1a1a1a] bg-[#111] text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button
            ...
            className="bg-[#ffffff] text-black rounded-full py-4 px-10 font-semibold text-base hover:opacity-90 transition-all duration-200"
          >
            Request Staff →
          </button>
          <button
