...
export default function BlogIndexPage() {
  ...
  useEffect(() => {
    ...
    document.title = 'Event Staffing Blog — Atlanta Tips & Guides | Vanda'
        ...
        <div className="space-y-8">
          {posts.map((post) => (
            ...
            >
              ...
              <h2 className="text-xl font-bold mb-2 hover:text-[#3ecf8e] transition-colors">
                {post.title}
              </h2>
              <p className="text-[#777] text-sm leading-relaxed">{post.description}</p>
              <div className="mt-4">
                <span className="text-[#3ecf8e] text-sm font-semibold">Read →</span>
              </div>
            </div>
          ))}
        </div>
